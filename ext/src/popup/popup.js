...
sendMessageButton.onclick = async function(e) {
  //Check if URL is gmail otherwise listening script isnt running
...
g_voi.onclick = async function() {
  ...
  await chrome.runtime.sendMessage({msg: "send_SMS",text:textfield}).then((res)=>response_label.textContent = res.status+res.message).catch((e)=>{document.getElementById('response').textContent = res.message});
